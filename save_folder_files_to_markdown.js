...
const FILE_EXTENSIONS_TO_INCLUDE = new Set(['.sh', '.py', '.js', '.ts', '.html', '.md', '.rst', '.json', '.yml', '.yaml']);
...
const MARKDOWN_TEMPLATE = `{relativePath}
\`\`\`
{content}
\`\`\`
...
function isHiddenDirectory(parts) {
    return parts.some(part => part.startsWith('.') && !ALLOWED_HIDDEN_DIRECTORIES.has(part));
}
...
for (const filePath of walkSync(baseWorkingPath)) {
    ...
    if (shouldSkipDirectory(parts) || (SKIP_HIDDEN_DIRECTORIES && isHiddenDirectory(parts))) {
        continue;
    }
    ...
    if (FILE_EXTENSIONS_TO_INCLUDE.has(path.extname(filePath))) {
        ...
        bigString += MARKDOWN_TEMPLATE.replace('{relativePath}', relativePath).replace('{content}', content);
    }
}
